...
Section
  ...
  ;Links pkt: URI's to PKT Electrum
  WriteRegStr HKCU "Software\Classes\pkt" "" "URL:PKT Protocol"
  WriteRegStr HKCU "Software\Classes\pkt" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\pkt" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\pkt\shell\open\command" "" "$\"$INSTDIR\electrum-${PRODUCT_VERSION}.exe$\" $\"%1$\""
